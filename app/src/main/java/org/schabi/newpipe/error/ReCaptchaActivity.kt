...
class ReCaptchaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recaptchaBinding = ActivityRecaptchaBinding.inflate(layoutInflater)
        setContentView(recaptchaBinding!!.getRoot())
        setSupportActionBar(recaptchaBinding!!.toolbar)

        val url: String = sanitizeRecaptchaUrl(intent.getStringExtra(RECAPTCHA_URL_EXTRA))
        // set return to Cancel by default
        setResult(RESULT_CANCELED)

        // enable Javascript
        val webSettings = recaptchaBinding!!.reCaptchaWebView.getSettings()
    ...
    private fun handleCookiesFromUrl(url: String?) {
        LogHandler.LogInDebugMode(TAG,
            "handleCookiesFromUrl: url=" + (url ?: "null"))
    ...
    private fun handleCookies(cookies: String?) {
        LogHandler.LogInDebugMode(TAG,
            "handleCookies: cookies=" + (cookies ?: "null"))
